...
import { FaPuzzlePiece, FaComments, FaBrain } from 'react-icons/fa';
...
const articles = [
  {
    ...
    readTime: '4 min read',
    headerBg: 'from-[#0F3460] to-[#1A4080]',
    accentBar: 'bg-[#0F3460]',
  },
  {
    icon: <FaComments />,
    ...
    readTime: '5 min read',
    headerBg: 'from-[#00ADB5] to-[#008B93]',
    accentBar: 'bg-[#00ADB5]',
  },
  {
    icon: <FaBrain />,
    ...
    readTime: '6 min read',
    headerBg: 'from-[#E94560] to-[#C93550]',
    accentBar: 'bg-[#E94560]',
  },
];
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-7">
          {articles.map((a, i) => (
            <motion.div
              key={i}
              className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] hover:shadow-[0_8px_40px_rgba(0,0,0,0.10)] border border-slate-100/80 overflow-hidden group cursor-pointer transition-all duration-300 hover:-translate-y-2 flex flex-col"
            ...
            >
              <div className={`h-44 bg-gradient-to-br ${a.headerBg} flex items-center justify-center relative overflow-hidden`}>
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${a.tagBg}`}>
                    {a.tag}
                  </span>
                </div>
              </div>

              <div className={`h-0.5 w-0 group-hover:w-full transition-all duration-500 ${a.accentBar}`} />

              <div className="p-6 flex flex-col gap-3 flex-1">
                <h3 className="text-lg font-bold text-[#0F3460] leading-snug group-hover:text-[#E94560] transition-colors duration-300">
                  {a.title}
                </h3>
                ...
                <button className="mt-1 text-[#00ADB5] font-semibold text-sm flex items-center gap-1 hover:gap-3 transition-all duration-300 group/btn w-fit">
                  Read More
